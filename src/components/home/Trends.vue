<template>
  <div class="flex flex-col text-primary">
    ...
    <div class="flex" v-for="trend in props.trendsList" :key="trend">
      <div class="basis-1/3 p-2 border flex">
        <div class="w-7 mr-2">
          <img :src="trend.thumb" :alt="trend.name" class="block" />
        </div>
        <router-link
          :to="{ name: 'Asset', params: { id: trend.id } }"
          class="font-bold link-text-primary"
        >
          {{ trend.symbol }}
        </router-link>
      </div>
      <div class="basis-1/3 p-2 border">{{ trend.name }}</div>
      <div class="basis-1/3 p-2 border">{{ calculateUsdPrice(trend.price_btc).toFixed(6) }}</div>
    </div>
  </div>
</template>
